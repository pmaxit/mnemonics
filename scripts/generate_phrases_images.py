...
def generate_image(client, word):
    prompt = (
        ...
        f"story to explain the phrase or collocation '{word}'. The comic should visually depict a scene and realistic "
        f"usage of the phrase '{word}' to help someone remember its meaning. Use a clear, engaging comic book style. "
...
def process_single_word(row, api_key):
    ...
    import uuid
    blob_name = f"phrase_{word.lower().replace(' ', '_').replace('/', '_')}.jpg"
...
def process_batch(limit=None, max_workers=3):
    load_dotenv()
    ...
    # Target only the newly uploaded phrases
    cursor.execute("""
        SELECT id, word, imageUrl 
        FROM Vocabulary 
        WHERE setIds LIKE '%phrases%' AND (imageUrl IS NULL OR imageUrl = '')
    """)
    ...
    print(f"Found {len(words_to_process)} phrases to process. Using {max_workers} threads.")
    
    if not words_to_process:
        print("No phrases need image generation.")
        return
