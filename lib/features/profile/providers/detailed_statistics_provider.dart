  ...
  try {
    ...
    if (params.categoryFilter != null && params.categoryFilter!.isNotEmpty) {
      combinedData = combinedData.where((item) {
        return item.word.category.toLowerCase() == params.categoryFilter!.toLowerCase();
      }).toList();
    }
    ...
    // Apply sorting
    combinedData = _sortWords(combinedData, params.sortBy, params.sortAscending);
    
    // Apply pagination
    final startIndex = params.page * params.pageSize;
    final endIndex = (startIndex + params.pageSize).clamp(0, combinedData.length);
...
List<WordWithUserData> _filterByStatType(List<WordWithUserData> words, WordStatType statType) {
  switch (statType) {
...
List<WordWithUserData> _sortWords(List<WordWithUserData> words, String sortBy, bool ascending) {
  words.sort((a, b) {
    int comparison = 0;
    ...
    switch (sortBy) {
      ...
      case 'lastReviewed':
        final aDate = a.userData?.lastReviewedAt ?? DateTime.fromMillisecondsSinceEpoch(0);
        final bDate = b.userData?.lastReviewedAt ?? DateTime.fromMillisecondsSinceEpoch(0);
...
@riverpod
Future<Map<String, CategoryStats>> categoryDetailedStats(CategoryDetailedStatsRef ref) async {
  try {
    final vocabularyRepo = ref.read(vocabularyRepositoryProvider);
    final userWordRepo = ref.read(userWordDataRepositoryProvider);
    ...
    for (final entry in wordsByCategory.entries) {
      ...
      final averageAccuracy = wordsWithData > 0 ? totalAccuracy / wordsWithData : 0.0;
...
@riverpod
Future<Map<String, DifficultyStats>> difficultyDetailedStats(DifficultyDetailedStatsRef ref) async {
  try {
    final vocabularyRepo = ref.read(vocabularyRepositoryProvider);
    final userWordRepo = ref.read(userWordDataRepositoryProvider);
